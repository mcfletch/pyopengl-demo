...
def LoadTexture(path):
    ...
    if (WidthPixels > glMaxTexDim) or (HeightPixels > glMaxTexDim):
        ...
        raise RuntimeError(
            "Texture image (%d by %d) is larger than supported by GL %d."
            % (WidthPixels, HeightPixels, glMaxTexDim)
        )

    # Create a raw string from the image data - data will be unsigned bytes
    # RGBpad, no stride (0), and first line is top of image (-1)
    pBits = (
        Picture.tostring("raw", "RGBX", 0, -1)
        if hasattr(Picture, "tostring")
        else Picture.tobytes("raw", "RGBX", 0, -1)
    )
...
def InitGL(Width, Height):  # We call this right after our OpenGL window is created.
    ...
    gCamera = glCamera()
    gCamera.m_MaxHeadingRate = 1.0
    # // Set our Maximum rates for the camera
...
def DrawGLInfo():
    ...
    String = "vLightSourceToCamera... = %.02f, %.02f, %.02f" % (
        gCamera.vLightSourceToCamera.i,
        gCamera.vLightSourceToCamera.j,
        gCamera.vLightSourceToCamera.k,
    )
    ...
    String = "vLightSourceToIntersect = %.02f, %.02f, %.02f" % (
        gCamera.vLightSourceToIntersect.i,
        gCamera.vLightSourceToIntersect.j,
        gCamera.vLightSourceToIntersect.k,
    )
    ...
    String = "%.02f, %.02f, %.02f, %.02f" % (
        gCamera.m_Frustum[0][0],
        gCamera.m_Frustum[0][1],
        gCamera.m_Frustum[0][2],
        gCamera.m_Frustum[0][3],
    )
    gFont.glPrintf(10, 300, 1, String)

    # // Print out the left clipping plane
    String = "%.02f, %.02f, %.02f, %.02f" % (
        gCamera.m_Frustum[1][0],
        gCamera.m_Frustum[1][1],
        gCamera.m_Frustum[1][2],
        gCamera.m_Frustum[1][3],
    )
    gFont.glPrintf(10, 280, 1, String)

    # // Print out the bottom clipping plane
    String = "%.02f, %.02f, %.02f, %.02f" % (
        gCamera.m_Frustum[2][0],
        gCamera.m_Frustum[2][1],
        gCamera.m_Frustum[2][2],
        gCamera.m_Frustum[2][3],
    )
    gFont.glPrintf(10, 260, 1, String)

    # // Print out the top clipping plane
    String = "%.02f, %.02f, %.02f, %.02f" % (
        gCamera.m_Frustum[3][0],
        gCamera.m_Frustum[3][1],
        gCamera.m_Frustum[3][2],
        gCamera.m_Frustum[3][3],
    )
    gFont.glPrintf(10, 240, 1, String)

    # // Print out the far clipping plane
    String = "%.02f, %.02f, %.02f, %.02f" % (
        gCamera.m_Frustum[4][0],
        gCamera.m_Frustum[4][1],
        gCamera.m_Frustum[4][2],
        gCamera.m_Frustum[4][3],
    )
    gFont.glPrintf(10, 220, 1, String)

    # // Print out the near clipping plane
    String = "%.02f, %.02f, %.02f, %.02f" % (
        gCamera.m_Frustum[5][0],
        gCamera.m_Frustum[5][1],
        gCamera.m_Frustum[5][2],
        gCamera.m_Frustum[5][3],
    )
...
def DrawGLScene():
    ...
    glRotatef((perf() * 1000.0) / 50.0, 0.3, 0.0, 0.0)
    glRotatef((perf() * 1000.0) / 50.0, 0.0, 0.5, 0.0)
    glCallList(gcylList)
    glPopMatrix()

    gCamera.SetPrespective()
    # // Set our perspective/oriention on the world
    gCamera.RenderLensFlare()
    # // Render the lens flare
    gCamera.UpdateFrustumFaster()
    # // Update the frustum as fast as possible.

    # // Check to see if info has been toggled by 1,2
    if gInfoOn:
        DrawGLInfo()
        # // Info is on so draw the GL information.
