...
class glCamera:
    ...
    def ChangeHeading(self, degrees):
        if fabs(degrees) < fabs(self.m_MaxHeadingRate):
            ...
            # // become inverted.
            if (
                self.m_PitchDegrees > 90
                and self.m_PitchDegrees < 270
                or (self.m_PitchDegrees < -90 and self.m_PitchDegrees > -270)
            ):
    ...
    def IsOccluded(self, p):
        ...
        # //get actual model view matrix
        projmatrix = glGetDoublev(GL_PROJECTION_MATRIX)
        # //get actual projiection matrix
    ...
    def RenderLensFlare(self):
        ...
        if self.SphereInFrustum(self.m_LightSourcePos, 1.0) == True:
            ...
            Length = (
                self.vLightSourceToCamera.Magnitude()
            ...
            if not self.IsOccluded(
                self.m_LightSourcePos
    ...
    def SetPrespective(self):
        ...
        glRotatef(self.m_HeadingDegrees, 0.0, 1.0, 0.0)
        # turn your head left/right (around y axe)
        glRotatef(self.m_PitchDegrees, 1.0, 0.0, 0.0)
        # nod your head up/down (around x axe)
