...
def InitGL(Width, Height):  # We call this right after our OpenGL window is created.
    ...
    camz = -19.0
    # // Set Camera Z Position To -19.0f
...
def DrawGLScene():
    ...
    # // Move To Our Camera Z Position

    glBegin(GL_QUADS)
    # // Back Wall
    glFogCoordfEXT(1.0)
    glTexCoord2f(0.0, 0.0)
    glVertex3f(-2.5, -2.5, -15.0)
    glFogCoordfEXT(1.0)
...
def keyPressed(*args):
    ...
    if (args[0] == GLUT_KEY_UP) and (camz < 14.0):
        camz += (
            milliseconds / 100.0
        )  # // Move Object Closer (Move Forwards Through Hallway)
    if args[0] == GLUT_KEY_DOWN and camz > -19.0:
        camz -= (
            milliseconds / 100.0
        )  # // Move Object Closer (Move Backwards Through Hallway)
