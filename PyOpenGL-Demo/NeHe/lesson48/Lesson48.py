...
def Upon_Click(button, button_state, cursor_x, cursor_y):
    ...
    if button == GLUT_RIGHT_BUTTON and button_state == GLUT_UP:
        # Right button click
        g_LastRot = Matrix3fSetIdentity()
        # // Reset Rotation
        g_ThisRot = Matrix3fSetIdentity()
        # // Reset Rotation
...
def Torus(MinorRadius, MajorRadius):
    ...
    for i in range(20):  # // Stacks
        for j in range(-1, 20):  # // Slices
            ...
            glNormal3f(
                sin(PI2 * (i % 20 + wrapFrac) / 20.0) * cosphi,
                sinphi,
                cos(PI2 * (i % 20 + wrapFrac) / 20.0) * cosphi,
            )
            glVertex3f(
                sin(PI2 * (i % 20 + wrapFrac) / 20.0) * r,
                MinorRadius * sinphi,
                cos(PI2 * (i % 20 + wrapFrac) / 20.0) * r,
            )

            glNormal3f(
                sin(PI2 * (i + 1 % 20 + wrapFrac) / 20.0) * cosphi,
                sinphi,
                cos(PI2 * (i + 1 % 20 + wrapFrac) / 20.0) * cosphi,
            )
            glVertex3f(
                sin(PI2 * (i + 1 % 20 + wrapFrac) / 20.0) * r,
                MinorRadius * sinphi,
