...
# // Flag To Let Us Know When It's Done

width = 128
# // Maze Width  (Must Be A Power Of 2)
height = 128
# // Maze Height (Must Be A Power Of 2)
...
def InitGL(Width, Height):  # We call this right after our OpenGL window is created.
    ...
    glEnable(GL_LIGHT0)
    # // Enable Light0 (Default GL Light)
...
def Update():
    ...
    if (
        ((mx > (width - 4) or tex_data[(((mx + 2) + (width * my)) * 3)] == 255))
        and ((mx < 2 or tex_data[(((mx - 2) + (width * my)) * 3)] == 255))
        and ((my > (height - 4) or tex_data[((mx + (width * (my + 2))) * 3)] == 255))
        and ((my < 2 or tex_data[((mx + (width * (my - 2))) * 3)] == 255))
    ):
        while True:  # // If We Are Trapped
            mx = (
                random.randint(0, (width / 2) - 1) * 2
            )  # // Pick A New Random X Position
            my = (
                random.randint(0, (height / 2) - 1) * 2
...
def DrawGLScene():
    ...
    xrot += milliseconds * 0.02
    # // Increase Rotation On The X-Axis
    yrot += milliseconds * 0.03
    # // Increase Rotation On The Y-Axis
    zrot += milliseconds * 0.015
    # // Increase Rotation On The Z-Axis
...
def keyPressed(*args):
    ...
    if args[0] == b" ":
        Reset()
        # // If So, Call Reset And Start A New Maze
